...
export default function LoginScreen({ navigation }: Props) {
  ...
  // 이메일 실시간 중복 체크 상태
  const [emailCheck, setEmailCheck] = useState<'idle'|'checking'|'ok'|'taken'|'error'|'invalid'>('idle');

  // 이메일이 변경될 때 디바운스로 실시간 중복 검사
  useEffect(() => {
    if (!emailFilled) { setEmailCheck('idle'); return; }
    ...
    const t = setTimeout(async () => {
      try {
        ...
        setEmailCheck((count ?? 0) > 0 ? 'taken' : 'ok');
      } catch {
        if (!cancelled) setEmailCheck('error');
      }
    }, 500);
    return () => { cancelled = true; clearTimeout(t); };
  }, [suEmail, emailFilled, emailValid]);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // 만료 시에는 입력칸을 숨기고(렌더링으로 제어), 타이머 라벨만 '시간 초과'로 표시
      // codeSent 상태는 유지하여 안내 문구와 타임아웃 표시가 가능하도록 함
    };
    tick();
  ...
  const onSendCode = async () => {
    ...
    if (codeSentAt && Date.now() - codeSentAt < 60_000) {
      const leftSec = Math.ceil((60_000 - (Date.now() - codeSentAt)) / 1000);
      appAlert('재전송 제한', `${leftSec}초 후 재전송을 눌러주세요.`);
      return;
    }
    try {
      // DB에 동일 전화번호가 이미 존재하면 전송 차단
      try {
        ...
        if ((dup.count ?? 0) > 0) {
          appAlert('전화번호 사용 중','이미 가입된 전화번호입니다. 다른 번호로 진행해주세요.');
          return;
        }
      } catch (_) {
        // RLS/네트워크 이슈 시에는 검사를 생략하고 계속 진행
      }
  ...
  if (pwConfirmMismatch) { appAlert('비밀번호 확인','비밀번호가 일치하지 않습니다.'); return; }
      ...
      if (codeSent) {
        if (otpExpiresAt && Date.now() > otpExpiresAt) {
          appAlert('인증 만료','3분이 지나 인증번호가 만료되었습니다. 재전송 후 다시 시도해주세요.');
          return;
        }
          ...
          >
            <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingVertical: 18, backgroundColor: BG }}>
                ...
                {emailFilled && emailValid && (
                  <Text
                    style={{
                      ...
                      marginTop: 4,
                      color:
                        emailCheck === 'taken' ? '#EF4444' :
                        emailCheck === 'ok' ? '#16A34A' :
                        emailCheck === 'error' ? SUBTLE : SUBTLE,
                    }}
                  >
                    {emailCheck === 'checking' && '사용 가능 여부 확인 중…'}
                    {emailCheck === 'ok' && '사용 가능한 이메일입니다.'}
                    {emailCheck === 'taken' && '이미 사용 중인 이메일입니다.'}
                    {emailCheck === 'error' && '실시간 중복 확인이 제한되어 있어요. 가입 단계에서 최종 확인됩니다.'}
                    {emailCheck === 'invalid' && ''}
                    {emailCheck === 'idle' && ''}
                  </Text>
                )}
              ...
              <View style={{ marginTop: 12 }}>
                ...
                {codeSent && (
                  <Text style={{ marginTop: 6, fontSize: 11, color: SUBTLE }}>
                    인증번호가 오지 않으면 1분 뒤 재전송을 시도해 주세요.
                  </Text>
                )}
              </View>
                ...
                <View style={{ marginTop: 12 }}>
                  ...
                  <View style={{ position: 'relative' }}>
                    <TextInput
                      ...
                      value={otp}
                      onChangeText={setOtp}
                      editable={!(otpExpiresAt && otpRemaining <= 0)}
                      style={{
                        height: 50,
                        paddingLeft: 14,
                        paddingRight: 64,
                      ...
                      <View style={{ position: 'absolute', right: 12, top: 0, bottom: 0, justifyContent: 'center' }}>
                        {otpRemaining > 0 ? (
                          <Text style={{ fontSize: 12, color: SUBTLE }}>
                            {Math.floor(otpRemaining/60).toString().padStart(2,'0')}:{(otpRemaining%60).toString().padStart(2,'0')}
                          </Text>
                        ) : (
                          <Text style={{ fontSize: 12, color: '#EF4444' }}>시간 초과</Text>
                        )}
                      </View>
                    )}
                  </View>
