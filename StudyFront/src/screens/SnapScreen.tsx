...
export default function SnapScreen({ navigation }: any) {
  ...
  useEffect(() => {
    if (extracting) {
      scanAnim.setValue(0);
      Animated.loop(
        Animated.timing(scanAnim, { toValue: 1, duration: 1300, useNativeDriver: false })
      ).start();
    } else {
  ...
  const confirmConvert = async () => {
    try {
      ...
      if (msg.includes('NotoSansKR') || msg.includes('폰트')) {
        showAlert('한글 폰트 필요', '한글이 포함된 PDF 생성을 위해 NotoSansKR 폰트를 추가해야 해요.\n프로젝트 assets/fonts/NotoSansKR-Regular.otf 추가 후 다시 시도해주세요.');
      } else if (msg.includes('Winansi cannot encode')) {
        showAlert('인코딩 오류', 'PDF 기본 폰트가 한글을 지원하지 않습니다. 한글 폰트를 추가해 주세요.');
      } else {
        showAlert('저장 실패', msg);
      }
    }
  ...
  return (
    <SafeAreaView style={{ flex:1, backgroundColor:BG }}>
      ...
      {/* 상단 바 */}
      <View style={{ paddingTop: insets.top, borderBottomWidth:1, borderColor:BORDER, backgroundColor:BG }}>
        ...
        {extractedText && !extracting && (
          <View>
            ...
            <View style={{ backgroundColor:SURFACE, borderWidth:1, borderColor:BORDER, borderRadius:18, padding:16, ...SHADOW }}>
              ...
              <View style={{ flexDirection:'row', marginTop:14 }}>
                <TouchableOpacity disabled={!extractedText} onPress={async ()=>{ if(!extractedText) return; Clipboard.setString(extractedText); const v = await Clipboard.getString(); if (v && v.length>0) { showAlert('복사됨','클립보드에 복사되었습니다.'); } else { showAlert('복사 실패','권한 또는 클립보드 이슈로 복사되지 않았어요.'); } }} style={{ flex:1, paddingVertical:12, borderRadius:12, backgroundColor:ACCENT_SOFT, borderWidth:1, borderColor:BORDER, alignItems:'center', marginRight:8, opacity: extractedText?1:0.4 }}>
              ...
              <TouchableOpacity onPress={reset} style={{ marginTop:12, alignSelf:'stretch', paddingVertical:12, borderRadius:12, backgroundColor:'#EEF1F6', alignItems:'center' }}>
                <Text style={{ fontSize:12, fontWeight:'700', color:SUBTLE }}>다른 파일 선택하기</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
      ...
      <Modal visible={namingVisible} transparent animationType="fade" onRequestClose={cancelNaming}>
        <View style={{ flex:1, backgroundColor:'rgba(0,0,0,0.28)', justifyContent:'center', padding:24 }}>
          <View style={{ backgroundColor:SURFACE, borderRadius:20, borderWidth:1, borderColor:BORDER, padding:20 }}>
            ...
            <TextInput value={pdfName} onChangeText={setPdfName} style={{ marginTop:14, borderWidth:1, borderColor:BORDER, borderRadius:12, paddingHorizontal:14, paddingVertical:10, backgroundColor:'#FFF', fontSize:14, color: INK }} placeholder="파일 이름" placeholderTextColor={SUBTLE} />
