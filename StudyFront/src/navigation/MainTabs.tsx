...
import { useFocusEffect, NavigationContainer, useIsFocused } from '@react-navigation/native';
import MaterialIcons from 'react-native-vector-icons/MaterialIcons';
import MaterialCommunityIcons from 'react-native-vector-icons/MaterialCommunityIcons';
import { listLibrary, getSignedUrl, currentUserId } from '../logic/library';
import AskScreen from '../screens/AskScreen';
import SnapScreen from '../screens/SnapScreen';
import AIProblemScreen from '../screens/AIProblemScreen';
import MyAIProblemsScreen from '../screens/MyAIProblemsScreen.tsx';
...
// 탭 화면 전환 슬라이드 애니메이션 HOC
const TAB_ORDER: Record<string, number> = { HomeStack: 0, Library: 1, Plan: 2, Settings: 3 };

function withTabSlide<P>(Wrapped: React.ComponentType<P>) {
  return function TabSlideWrapper(props: P & { route?: { name?: string } }) {
    const isFocused = useIsFocused();
    const trans = React.useRef(new Animated.Value(0)).current;
    const width = Dimensions.get('window').width;

    React.useEffect(() => {
      const sub = DeviceEventEmitter.addListener('tabWillChange', (payload: { fromIndex: number; toIndex: number }) => {
        const name = (props as any)?.route?.name as string | undefined;
        if (!name) return;
        const myIndex = TAB_ORDER[name];
        if (myIndex === payload.toIndex) {
          const dir = payload.toIndex > payload.fromIndex ? 1 : -1; // 앞으로(큰 인덱스) → 오른쪽에서 들어옴
          trans.setValue(dir * width);
        }
      });
      return () => sub.remove();
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [(props as any)?.route?.name]);
...
function HomeScreen({ navigation }: any) {
  ...
  const loadGoals = React.useCallback(async () => {
    try {
      ...
      if (uidNow) {
        try {
          const flag = await AsyncStorage.getItem(`goalsIntroDismissed:${uidNow}`);
          setIntroDismissed(flag === '1');
        } catch {}
      } else {
      ...
      const { data, error } = await supabase
        .from('goals')
        .select('id,text,done,inserted_at')
        .order('inserted_at', { ascending: true });
      if (error) { showToast('목표 불러오기 실패'); return; }
      setGoals((data || []).map((r: any) => ({ id: r.id, text: r.text, done: r.done })));
    } catch (e) {
      showToast('네트워크 오류');
    } finally {
  ...
  const markIntroDismissed = React.useCallback(async () => {
    if (!uid || introDismissed) return;
    setIntroDismissed(true);
    try { await AsyncStorage.setItem(`goalsIntroDismissed:${uid}`, '1'); } catch {}
  }, [uid, introDismissed]);

  const toggleGoal = async (id: string) => {
    if (id === '__intro__') { markIntroDismissed(); return; }
    const target = goals.find(g => g.id === id);
    if (!target) return;
    const next = !target.done;
    setGoals(gs => gs.map(it => it.id === id ? { ...it, done: next } : it)); // optimistic
    const { error } = await supabase.from('goals').update({ done: next }).eq('id', id);
    if (error) { showToast('업데이트 실패'); setGoals(gs => gs.map(it => it.id === id ? { ...it, done: !next } : it)); } else { markIntroDismissed(); }
  };

  const addGoal = async () => {
    try {
      const { data, error } = await supabase.from('goals').insert({ text: '새 목표', done: false }).select('id,text,done').single();
      if (error) { showToast('추가 실패'); return; }
      if (data) setGoals(g => [...g, { id: data.id, text: data.text, done: data.done }]);
      markIntroDismissed();
    } catch {
      showToast('네트워크 오류');
    }
  ...
  const saveGoalText = async (id: string, text: string) => {
    if (id === '__intro__') { markIntroDismissed(); return; }
    const { error } = await supabase.from('goals').update({ text }).eq('id', id);
    if (error) showToast('저장 실패'); else markIntroDismissed();
  };
  ...
  const deleteGoal = async (id: string) => {
    ...
    if (error) { showToast('삭제 실패'); setGoals(prev); } else { markIntroDismissed(); }
  };

  // 편집 모달 닫을 때 남은 텍스트 일괄 저장 (onEndEditing 누락 대비)
  const persistAllGoalTexts = React.useCallback(async () => {
    try {
      const updates = goals.filter(g=>g.id!=='__intro__').map(g => supabase.from('goals').update({ text: g.text }).eq('id', g.id));
      const results = await Promise.allSettled(updates);
      if (results.some(r => r.status === 'rejected')) showToast('일부 저장 실패');
    } catch {
      showToast('저장 중 오류');
    }
  }, [goals]);
  ...
  const onPressCard = (type: 'snap' | 'ask' | 'quiz' | 'ai') => {
    ...
    if (type === 'ai') { navigation.navigate('AIProblem'); return; }
  };
  ...
  const displayGoals = React.useMemo(() => {
    if (goals.length === 0 && !introDismissed) {
      return [{ id: '__intro__', text: '오늘의 목표를 추가해 보세요', done: false } as Goal];
    }
    return goals;
  }, [goals, introDismissed]);
  const visibleGoals = displayGoals.slice(0,3);
  // 프로필 이름/이메일 로드
  ...
  <View style={{ backgroundColor: SURFACE, borderRadius:20, borderWidth:1, borderColor:BORDER, padding:18, marginBottom:20, alignSelf:'center', width:'90%', ...SHADOW }}>
          ...
          {displayGoals.length > 3 && (
            <TouchableOpacity onPress={() => setShowAllModal(true)} style={{ paddingVertical:4 }}>
              <Text style={{ fontSize:12, color: SUBTLE }}>… {displayGoals.length - 3}개 더 있음</Text>
            </TouchableOpacity>
          )}
        </View>
        ...
        <View style={{ flex:1, backgroundColor:'rgba(0,0,0,0.25)', justifyContent:'center', padding:24 }}>
          <View style={{ backgroundColor: SURFACE, borderRadius:20, borderWidth:1, borderColor:BORDER, padding:18, maxHeight:'80%' }}>
            ...
            <ScrollView style={{ maxHeight:300 }}>
              {displayGoals.map(g => (
                <View key={g.id} style={{ flexDirection:'row', alignItems:'center', marginBottom:10 }}>
                      <TouchableOpacity onPress={() => toggleGoal(g.id)} disabled={g.id==='__intro__'} style={{ width:28, height:28, borderRadius:8, borderWidth:1, borderColor: g.done ? SUCCESS : BORDER, backgroundColor: g.done ? SUCCESS : '#fff', alignItems:'center', justifyContent:'center', marginRight:10, opacity: g.id==='__intro__' ? 0.5 : 1 }}>
                        {g.done && <MaterialIcons name="check" size={18} color="#fff" />}
                  </TouchableOpacity>
                  <TextInput
                    value={g.text}
                    onChangeText={tx => updateGoalText(g.id, tx)}
                    onEndEditing={(e) => saveGoalText(g.id, e.nativeEvent.text)}
                    editable={g.id !== '__intro__'}
                    style={{ flex:1, fontSize:14, paddingVertical:6, paddingHorizontal:10, borderWidth:1, borderColor:BORDER, borderRadius:10, backgroundColor:'#fff' }}
                    placeholder="목표 입력"
                  />
                  <TouchableOpacity onPress={() => deleteGoal(g.id)} disabled={g.id==='__intro__'} style={{ marginLeft:8, padding:8, opacity: g.id==='__intro__' ? 0.5 : 1 }}>
            ...
            <View style={{ flexDirection:'row', marginTop:12 }}>
              ...
              <TouchableOpacity onPress={closeEditModal} style={{ flex:1, paddingVertical:12, borderRadius:12, backgroundColor: ACCENT, alignItems:'center' }}>
...
// Define HomeStack to include HomeMain, Snap, Ask screens
function HomeStack() {
  return (
    <HomeStackNav.Navigator screenOptions={{ headerShown: false }}>
      ...
      <HomeStackNav.Screen name="AIProblem" component={AIProblemScreen} />
      <HomeStackNav.Screen name="MyAIProblems" component={MyAIProblemsScreen} />
    </HomeStackNav.Navigator>
  );
}

// Minimal Settings screen placeholder consistent with app style
function SettingsScreen({ navigation }: any) {
  const toastOpacity = React.useRef(new Animated.Value(0)).current;
  const [toast, setToast] = React.useState('');
  const width = Dimensions.get('window').width;
  const currentPlan: 'free' | 'standard' | 'premium' = 'premium';
  const plans: Array<{key:'free'|'standard'|'premium'; label:string; desc:string; color?:string}> = [
  ...
  const [renderingSection, setRenderingSection] = React.useState<SettingsSection|null>(null); // 애니 표시용
  ...
  const renderSection = (pName: string, pEmail: string) => {
    ...
    if (renderingSection === 'account') return (
      <Card>
        ...
        <TouchableOpacity onPress={doSignOut} style={{ paddingVertical:12, marginTop:4 }}>
          <Text style={{ color: DANGER, fontSize:13, fontWeight:'700' }}>로그아웃</Text>
        </TouchableOpacity>
        ...
        <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center', paddingVertical:8 }}>
          <View>
            ...
            <TouchableOpacity onPress={()=>showToast('시간 설정 준비중')} style={{ marginTop:4 }}>
              <Text style={{ fontSize:11, color:ACCENT, fontWeight:'700' }}>오전 8:30</Text>
            </TouchableOpacity>
          </View>
          <Toggle value={dailyReminder} onToggle={()=>setDailyReminder(v=>!v)} />
        </View>
        <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center', paddingVertical:8 }}>
          <Text style={{ fontSize:13, fontWeight:'600', color:INK }}>주간 학습 리포트</Text>
    ...
    if (renderingSection === 'about') return (
      <Card>
        ...
        <TouchableOpacity onPress={()=>showToast('문의/피드백 준비중')} style={{ paddingTop:8 }}>
  ...
  // 안드로이드 하드웨어 뒤로가기: 섹션 내부면 루트로
  React.useEffect(() => {
    const onHardwareBack = () => {
      if (renderingSection) { goBack(); return true; }
      return false;
    };
  ...
  return (
    <SafeAreaView style={{ flex:1, backgroundColor: BG }}>
      <StatusBar barStyle="dark-content" backgroundColor={BG} />
      <TopBar
        title={section ? sectionTitleMap[section] : '설정'}
        onBack={section ? goBack : undefined}
      />
      <View style={{ flex:1 }}>
        {/* 루트 목록 */}
        {!renderingSection && (
...
// Replaced LibraryScreen with real storage listing version (첫 번째 정의 — 사용됨)
...
function LibraryScreen() {
  ...
  const onPressItem = async (name: string) => {
    try {
      ...
      const path = name.includes('/') ? name : `${uid}/${name}`;
...
function PlanScreen() {
  ...
  const loadSchedules = React.useCallback(async ()=>{
    try {
      ...
      const { data, error } = await supabase
        .from('schedules')
        .select('id,title,descr,date,favorite,alarm_time')
        .order('date', { ascending: true });
      if (error) { showToast('일정 불러오기 실패'); return; }
      const mapped = (data||[]).map((r:any)=> ({ id:r.id, title:r.title, desc:r.descr||'', date:r.date, favorite:!!r.favorite, alarmTime: r.alarm_time || null }));
  ...
  const addSchedule = async () => {
    ...
    const payload = { title: title.trim(), descr: desc.trim(), date: dateStr, favorite: false };
    const { data, error } = await supabase.from('schedules').insert(payload).select('id,title,descr,date,favorite,alarm_time').single();
    if (error) { showToast('추가 실패'); return; }
    const row = { id: data.id, title: data.title, desc: data.descr||'', date: data.date, favorite: !!data.favorite, alarmTime: data.alarm_time || null } as Schedule;
            ...
            <View style={{ backgroundColor:SURFACE, borderRadius:20, borderWidth:1, borderColor:BORDER, padding:20, maxHeight:'85%' }}>
              <View style={{ flexDirection:'row', alignItems:'center' }}>
                ...
                <TouchableOpacity onPress={async ()=>{
                  ...
                  setItems(prev => sortItems(prev.map(it => it.id===detail.id? { ...it, favorite: next }:it)));
                  setDetail(d=> d? { ...d, favorite: next }:d);
                  const { error } = await supabase.from('schedules').update({ favorite: next }).eq('id', detail.id);
                  if (error) { showToast('즐겨찾기 실패'); setItems(prev => sortItems(prev.map(it => it.id===detail.id? { ...it, favorite: !next }:it))); setDetail(d=> d? { ...d, favorite: !next }:d); }
              ...
              <View style={{ flexDirection:'row', marginTop:24 }}>
                <TouchableOpacity onPress={async ()=>{
                  const prev = items;
                  setItems(p=> p.filter(it=>it.id!==detail.id));
                  const { error } = await supabase.from('schedules').delete().eq('id', detail.id);
                  if (error) { showToast('삭제 실패'); setItems(prev); } else { showToast('삭제됨'); }
                  setDetail(null);
...
// 광고 캐러셀 컴포넌트 (두 번째 정의 — 보존을 위해 이름 변경)
function AdCarousel2() {
  const width = Dimensions.get('window').width;
  const horizontalPadding = 20; // 좌우 패딩 (홈 섹션 전반)
  const cardWidth = width - horizontalPadding * 2;
  const ads: Array<{id:string; title:string; subtitle:string; accent?:string; bg?:string}> = [
...
// Replaced LibraryScreen with real storage listing version (두 번째 정의 — 보존을 위해 이름 변경)
function TopBarSimple2({ title }: { title: string }) {
  return (
    <View style={{ backgroundColor: BG, borderBottomColor: BORDER, borderBottomWidth: 1 }}>
      <View style={{ paddingTop: Platform.OS === 'android' ? (StatusBar.currentHeight ?? 0) : 0, backgroundColor: BG }}>
        <View style={{ paddingHorizontal: 16, paddingVertical: 10, alignItems: 'center', justifyContent:'center' }}>
...
type Schedule2 = { id:string; title:string; desc:string; date:string; favorite?:boolean; alarmTime?:string|null };
...
// HomeStack definition remains as originally in this file/project

// 레거시 내비게이션 컨테이너 포함 버전 (보존). 사용하지 않도록 export 이름 변경
function MainTabsLegacy() {
  return (
    <NavigationContainer>
      <Tab.Navigator
        screenOptions={{
          tabBarStyle: { backgroundColor: SURFACE, borderTopWidth: 0, elevation: 0 },
          headerShown: false,
        }}
      >
        <Tab.Screen
...
// 새 MainTabs: RootNavigator/App.tsx에서 NavigationContainer를 이미 감싸므로 여기선 Tab만 렌더
export default function MainTabs() {
  const currentIndexRef = React.useRef(0);
  const emitWillChange = (fromIndex: number, toIndex: number) => {
    DeviceEventEmitter.emit('tabWillChange', { fromIndex, toIndex });
  };

  const HomeStackWithSlide = React.useMemo(() => withTabSlide(HomeStack), []);
  const LibraryWithSlide = React.useMemo(() => withTabSlide(LibraryScreen), []);
  const PlanWithSlide = React.useMemo(() => withTabSlide(PlanScreen), []);
  const SettingsWithSlide = React.useMemo(() => withTabSlide(SettingsScreen), []);
    ...
    >
      <Tab.Screen
        ...
        listeners={({ route, navigation }) => ({
          tabPress: () => {
            const to = TAB_ORDER[route.name] ?? 0;
            emitWillChange(currentIndexRef.current, to);
            DeviceEventEmitter.emit('homeTabPress');
            (navigation as any).navigate('HomeStack', { screen: 'HomeMain' });
          },
          focus: () => { currentIndexRef.current = TAB_ORDER[route.name] ?? 0; },
        })}
        options={{
          tabBarLabel: '홈',
          tabBarIcon: ({ color, size }) => <MaterialIcons name="home" color={color} size={size} />,
      ...
      >
        {(props) => <HomeStackWithSlide {...props} />}
      </Tab.Screen>
      ...
      <Tab.Screen
        ...
        listeners={({ route }) => ({
          tabPress: () => {
            const to = TAB_ORDER[route.name] ?? 1;
            emitWillChange(currentIndexRef.current, to);
          },
          focus: () => { currentIndexRef.current = TAB_ORDER[route.name] ?? 1; },
        })}
        options={{
          tabBarLabel: '자료함',
          tabBarIcon: ({ color, size }) => <MaterialIcons name="folder" color={color} size={size} />,
      ...
      <Tab.Screen
        ...
        listeners={({ route }) => ({
          tabPress: () => {
            const to = TAB_ORDER[route.name] ?? 2;
            emitWillChange(currentIndexRef.current, to);
          },
          focus: () => { currentIndexRef.current = TAB_ORDER[route.name] ?? 2; },
        })}
        options={{
          tabBarLabel: '플랜',
          tabBarIcon: ({ color, size }) => <MaterialIcons name="event" color={color} size={size} />,
      ...
      <Tab.Screen
        ...
        listeners={({ route }) => ({
          tabPress: () => {
            const to = TAB_ORDER[route.name] ?? 3;
            emitWillChange(currentIndexRef.current, to);
          },
          focus: () => { currentIndexRef.current = TAB_ORDER[route.name] ?? 3; },
        })}
        options={{
          tabBarLabel: '설정',
          tabBarIcon: ({ color, size }) => <MaterialIcons name="settings" color={color} size={size} />,
